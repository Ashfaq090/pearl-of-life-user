...
import { dateToIsoString, isoStringToDateObj, RELATION_LIST } from 'src/app/constants/app.constant';
...
export class ManageMemoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.memoriesForm = this.formBuilder.group({
      ...
      folder_id: [{ value: this.data?.item?.folder_id || '', disabled: this.data?.item?.folder_id }, Validators.required],
      date: [null],
      description: [''],
    });
